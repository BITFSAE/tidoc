...
一个 zip 包，内含：报账人清单与条目归属 + 规范命名的 PDF / 截图 / 查验单附件 + 汇总信息文本 + HMAC 签名清单。文件与信息**绑定**，可整体导出、他人整体导入还原。绑定包版本 2 开始显式保存身份清单；旧版本中每条携带的姓名和审核人仍可兼容恢复。备注与标签按本机设置决定是否写入包内。
