...
3. **核对与整理**：识别提醒可批量重新识别，也可用阿里云云识别补齐明细的数量、规格（选中条目点「云识别」，或识别提醒视图一键识别全部），不必删除条目重建；按抬头、报账人、状态、日期、金额、标签和备注筛选。卡片上的报账人、批次可直接编辑，材料按钮右键可打开已有文件；批次栏用「在办 / 已收档」切换工作面，点击批次右侧「⋯」可编辑批次、填写批次备注或归档，已收档批次可恢复到在办。设置 → 扩展可修改新建默认抬头，并分别设置付款截图、实物图、查验单和实付金额是否必需；发票固定必需，默认要求付款截图、查验单和实付金额，实物图可选。
