...
4. 条目卡片的「查验」会打开国家税务总局官网，并自动填入发票号码、日期和价税合计；用户在官网填写验证码（通常不区分大小写）。出现查验明细后，回到 tidoc 点击「保存到条目」，查验单 PDF 会自动归入对应条目；已有 PDF 也可以直接上传。
...
- **在线查验**：从条目卡片或详情打开全国增值税发票查验平台，预填发票号码、日期和价税合计；验证码由用户在官网填写。出现查验明细后，点击「保存到条目」即可生成横版 PDF、归入对应条目并更新材料状态；macOS 和 Windows 均使用系统 WebView 的 PDF 能力，已有查验单也可直接上传。
