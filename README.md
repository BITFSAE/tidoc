...
核心安装包不含打印导出组件；可在「设置 → 组件与更新」中安装，也可在开发环境执行 `pip install -r requirements-print.txt`。生成报账说明或验收单 Word 前，还需在「收款信息」中维护姓名、学号、电话、开户行和卡号。

完整操作演示见 [Bilibili 使用说明视频](https://www.bilibili.com/video/BV1XN3q69EPi/)。

## 工作流

1. **导入发票**：PDF 是创建条目的必要材料，XML 用于提高识别准确度，不能单独创建条目。支持单条、文件夹、多选、拖拽和粘贴导入。
...
3. **核对与整理**：识别提醒可批量重新识别，不必删除条目重建；按抬头、报账人、状态、日期、金额、标签和备注筛选。卡片上的报账人、批次可直接编辑，材料按钮右键可打开已有文件；批次栏用「在办 / 已收档」切换工作面，点击批次右侧「⋯」可编辑批次、填写批次备注或归档，已收档批次可恢复到在办。设置 → 扩展可修改新建默认抬头，并分别设置付款截图、实物图、查验单和实付金额是否必需；发票固定必需，默认要求付款截图、查验单和实付金额，实物图可选。
...
- 发票识别以 XML 优先、PDF 文本兜底，兼容常见数电发票的错位、分列明细和折扣行；缺明细、金额不闭合、抬头异常等会形成识别提醒。
- 识别提醒与材料齐备是两套状态；严重问题会单独标出。全库按发票号或文件内容拦截重复发票，附件类型、发票归属和导入事务也会校验。
- 不依赖云端通用 OCR。系统 OCR 只用于付款截图金额和查验单发票号的轻量兜底，发票本身仍以 XML / PDF 文本解析为主。
- **报账人**是发票归属，只需姓名和审核人；**收款信息**只供打印 Word 使用。条目备注记录单张发票，标签用于筛选，附件备注只描述具体文件。

## 网络与更新

应用不做默认后台联网；税务查验、外部链接和更新均由用户触发。「启动后检查更新」默认关闭，开启后每 24 小时最多检查一次，且不会自动下载或安装。设置中的「软件与组件」分别显示核心和打印组件状态，下载后校验 SHA256；组件缺失或损坏时可直接修复或重新安装。
...
从源码运行时优先使用当前工作区的 `tidoc_print`；发布版只调用已安装并校验过的独立组件。
...
- [x] 发票解析、识别提醒、材料绑定与在线查验辅助
- [x] 条目、筛选、标签、批次（在办 / 已收档）、默认抬头和材料要求配置
- [x] `.tidoc` 绑定包、Excel / 附件导出与 HMAC 校验
- [x] 可选打印导出组件（PDF / Word）
