...
- 系统“另存为 PDF”对话框不提供应用可可靠预设的输出路径。会话默认监测下载、桌面和文档目录，用户也可在查验弹窗选择并记住一个额外目录；成品保存到任一监测目录后，会话会核对查验单类型和发票号，再由附件仓库复制到发起查验的条目、刷新材料状态并关闭查验窗口。打印前把顶层文档标题设为“查验单-发票号码”，让系统保存对话框得到可区分的建议文件名；不要求用户回到 tidoc 再点击保存。手动上传已有 PDF 保留为明确兜底。
