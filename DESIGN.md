...
- `title` **抬头**：默认为 `北京理工大学` / `北京理工大学教育基金会`，可在设置中按学校 / 单位维护 —— 强隔离字段，见第 7 节
- **识别得到、默认只读**的字段：
  - `invoice_no` 发票号码
  - `invoice_date` 发票日期
  - `seller` 销售方 / 来源出货厂商
  - `total` 价税合计
  - `buyer_name` 购买方抬头、`buyer_tax_id` 税号。税号校验按设置里的「抬头与税号」配置执行，默认值：北京理工大学 `12100000400009127B`，北京理工大学教育基金会 `53100000500021676K`
...
报账抬头与购买方税号在设置 → 扩展的「抬头与税号」中维护，默认内置 `北京理工大学` 与 `北京理工大学教育基金会`，其他学校 / 单位可自行增删。不同抬头在**数据、列表分区、汇总、绑定包、打印合并**各环节全程分开，绝不混合：

- 界面用不同分区 / 配色区分（默认两个抬头保留专属配色，其他抬头用通用样式）。
- 汇总、绑定包、打印件默认按抬头拆分文件。
- 打印导出组件跨人合并时，也绝不把同一配置内不同抬头的材料混进同一份打印件。
- 识别时按当前生效的配置核对购买方税号与抬头范围。已配置抬头的税号缺失或不符、已知税号与识别抬头矛盾、抬头不在配置内，均形成「识别提醒」；清空配置后不做抬头范围提醒。税号比较忽略空白和常见分隔符，但界面保留识别原值便于核对。
...
Windows 安装器把 `.tidoc` 注册为 Tidoc 绑定包：资源管理器中显示应用图标，双击直接启动 Tidoc 并进入该包的导入预览；卸载时自动清除关联。

### 8.7 分类 / 筛选 / 搜索
按抬头、报账人、日期、销售方、金额区间、状态、条目标签、条目备注、付款截图数量和关键词过滤，也可通过批次栏按「在办」、批次、「未进批次」或「已归档」聚焦；付款截图数量提供「多张（2 张及以上）」快速条件。筛选条件直接通过对应控件的浅蓝激活态表达，高级条件同时点亮具体筛选框和「高级」入口，不在工具条下方重复生成条件标签；下拉控件统一箭头、边框和交互状态。抬头下拉以设置里配置的抬头为固定项，并动态补入数据库中实际出现的其他抬头，存在未配置抬头时在筛选控件显示轻量提示点。常用视图（全部、待补材料、识别提醒、已修改、齐备）作为筛选条内的快捷条件，不拆成左侧多个“页面”。其中“已修改”仅指实付金额与发票总金额不一致，其他字段的修改仍正常留痕，但不进入该视图；悬浮徽标显示实付以及同时存在的其他字段修改详情。通过卡片勾选、Shift 范围选择和 Cmd/Ctrl+A 选中条目后，再做批量修改报账人、汇总、导出、打印或删除，重新识别等长任务按条目显示完成进度。
...
设置 → 扩展集中维护进阶项：「抬头与税号」按学校 / 单位增删报账抬头和购买方税号（默认内置北理工与教育基金会），保存后识别、校验和各处抬头下拉立即按新配置执行；「新建默认抬头」与「材料要求」同处保存——发票固定必需，付款截图、实物图、查验单、实付金额可分别设为必需或可选，默认要求付款截图、查验单和实付金额，切换后立即重算已有条目的状态。设置里其余外观主题等控件统一使用下拉选择。
...
- 深色主题延续安静、密集的工作台风格：画布用接近 VS Code 的中性近黑（略带冷调），表面由画布、面板、浮层逐级提亮，去掉整体色雾；状态色分别保留抬头、齐备、提醒、严重问题和人工修改的业务含义，不采用简单反色。详情里必需但未上传的材料组用琥珀色虚线框提示，重复的「待补」文字提示不再出现。
