...
报账抬头与购买方税号在设置的「抬头与税号」中维护，默认内置 `北京理工大学` 与 `北京理工大学教育基金会`，其他学校 / 单位可自行增删。不同抬头在**数据、列表分区、汇总、绑定包、打印合并**各环节全程分开，绝不混合：
...
设置 → 抬头与税号维护报账抬头和购买方税号（默认内置北理工与教育基金会，供其他学校 / 单位使用时增删；保存后识别、校验和各处抬头下拉立即按新配置执行）。设置 → 扩展保存新建默认抬头和材料要求；发票固定必需，付款截图、实物图、查验单、实付金额可分别设为必需或可选，默认要求付款截图、查验单和实付金额，切换后立即重算已有条目的状态。
