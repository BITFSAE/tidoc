...
一个 zip 包，内含：报账人清单与条目归属 + 规范命名的 PDF / 截图 / 查验单附件 + 汇总信息文本 + HMAC 签名清单。文件与信息**绑定**，可整体导出、他人整体导入还原。绑定包版本 2 开始显式保存身份清单；版本 3 开始保存阿里云原始响应、解析快照、待确认状态和自动补齐 / 历史修正快照，导入结果不计入接收方本机调用次数。旧版本中每条携带的姓名和审核人仍可兼容恢复。备注与标签按本机设置决定是否写入包内。
...
**结果持久化（防重复计费）**
- 每次调用（含失败）都追加写入 `ocr_results` 表：原始 `data` JSON 全文、解析快照、发票文件 sha256、金额闭合结果、待确认差异列表、时间；自动补齐时同时保存字段及明细的补齐前 / 补齐后快照，详情直接展示。
...
- 软件优先：销售方和明细已有值时不由阿里云覆盖。销售方全角 / 半角标点视为一致；规格型号不参与差异判断，也不由云识别补齐。明细行数能对齐、金额闭合且不是 XML 来源时，只补名称、单位、数量、金额的空单元格；其他明细结果保留供手动比对和采用。
- 待确认：发票号、日期、总额、购买方相关字段的非空冲突，以及 XML 来源或金额闭合失败的明细，只在详情「阿里云识别」折叠区展示，由用户逐项或整套采用；条目卡片挂 OCR 徽标直到处理完。销售方和已闭合 PDF 明细的冲突标成「保留软件值」，不产生待确认徽标。
