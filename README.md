...
4. **交换、导出与打印**：选中条目后可创建批次、导出 `.tidoc` 绑定包、总览 Excel 或附件整理包。绑定包导入前可预览并调整报账人、审核人、批次和标签，重复发票跳过并校验 HMAC；阿里云识别结果和“已识别 / 待确认”状态会随包恢复。打印组件默认按条目合并发票、付款截图和查验单，也支持按材料分开导出，并生成报账说明、验收单 Word；若当前聚焦某个批次，会自动带入该批次已保存的批次备注；Word 与 PDF 按抬头隔离。
