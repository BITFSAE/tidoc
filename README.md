...
4. 条目卡片的「查验」会打开国家税务总局官网，并自动填入、确认发票号码、日期和价税合计；日期日历会自动收起。用户填写验证码并查验成功后，直接点击官网「打印」，用系统打印窗口横向另存为 PDF；下载、桌面、文档目录默认会被监测，也可在查验弹窗为本次及后续查验选择其他自动归档目录。PDF 默认使用“查验单-发票号码”命名，保存后 tidoc 会自动复制到对应条目并更新材料状态，无需回到弹窗再点一次保存。已有 PDF 也可以直接上传。
...
- **在线查验**：从条目卡片或详情打开全国增值税发票查验平台，预填并触发官网校验发票号码、日期和价税合计，自动收起日期日历；验证码由用户在官网填写。查验成功后使用官网「打印」和系统打印窗口横向另存为 PDF，建议文件名自动带发票号码；下载、桌面、文档及用户自选目录中的新 PDF 会自动复制到对应条目并更新材料状态，无需回到 tidoc 再确认。已有查验单也可直接上传。
