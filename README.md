...
- **材料整理**：付款截图、查验单可在条目详情添加，也可拖到条目卡片或材料区绑定；付款截图默认用系统本地 OCR 尝试识别实付金额，设置中可关闭。OCR 开启时，金额只对应一个条目才自动归入；关闭后不读取截图金额并直接手动绑定。删除最后一张付款截图时，如果实付仍是 OCR 自动值且没有被人工改过，会恢复为发票总金额。“实付默认等于发票金额”是独立设置：开启时新条目自动填写发票金额，关闭时留空待确认，已有条目不受切换影响。
