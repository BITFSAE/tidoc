...
- 软件启动完成后默认读取公开 manifest，每 24 小时最多检查一次；用户可在设置关闭。自动检查只显示设置旁的下载动作，不会自动下载或安装。
- 自动发现核心更新时只显示顶栏下载动作；升级后的首次启动用简短提示显示升级前后的版本号，不再打断工作流弹出更新说明。完整使用指南仍可从设置打开。
...
- 核心下载进度同时显示在顶栏动作和更新弹窗；组件安装过程仍在设置弹窗显示，完成后立即刷新状态。
