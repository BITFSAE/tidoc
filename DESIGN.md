...
- 发票查验单 PDF：可单独上传，也可从条目卡片 / 详情进入「在线查验」。在线流程预填数电发票所需的发票号码、日期与价税合计；官网验证码由用户读取和填写，tidoc 不代填、不绕过验证。界面只保留验证码和保存动作两条必要提示。
- 应用不会后台打开官网或自动检查；只有用户明确点击「打开查验官网」才联网。查验窗口不向官网暴露主窗口的 JS API。考虑到部分省级验证码接口使用会被 WebView 拒绝的历史证书，查验窗口存续期间临时启用 pywebview 的证书错误放行，最后一个查验窗口关闭后恢复启动前设置；这属于内部兼容措施，不在日常界面暴露。
- 官网结果页和旧版 PrintArea 目标位于 iframe 中。查验窗口持续发现同源结果 frame，将官网选定的查验明细复制到顶层导出容器；用户点击「保存到条目」后，macOS 通过 WKWebView `createPDF`、Windows 通过 WebView2 `PrintToPdfAsync` 生成横版暂存 PDF，整个过程复用已经显示的结果，不重新提交查验。
- 生成的 PDF 由附件仓库复制到发起查验的条目，随后删除暂存文件、刷新材料状态并关闭查验窗口。手动上传已有 PDF 保留为明确兜底；官网自行下载或打印出的 PDF 仍可通过会话目录监测自动归档。
...
- 绑定包 `.tidoc`：给 tidoc 之间互相导入继续整理，保留条目、附件、修改历史与签名校验。导入按整个包原子处理，任一条目或附件失败时回滚本次全部写入；发票号或发票文件摘要已存在时跳过重复条目。用户明确确认接收完整性异常的包后，导入条目标记为严重问题，不以普通条目静默落库。
