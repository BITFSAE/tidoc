...
- 工具条统一承载搜索、抬头 / 报账人筛选、状态预设、高级筛选和排序；批次归属与“未进批次”浏览通过批次栏和条目卡片直接管理。
...
- 批次栏以文件夹式入口呈现「全部条目」「未进批次」与各批次，可打开、重命名、归档和整批处理；单条卡片可直接新建、切换或清空批次归属。
...
### 8.7 分类 / 筛选 / 搜索
按抬头、报账人、日期、销售方、金额区间、状态、条目标签、条目备注、关键词过滤，也可通过批次栏按批次或“未进批次”聚焦；抬头下拉以两个内置抬头为固定项，并动态补入数据库中实际出现的其他抬头，存在第三方抬头时在筛选控件显示轻量提示点。常用视图（全部、待补材料、识别提醒、已修改、齐备）作为筛选条内的快捷条件，不拆成左侧多个“页面”。其中“已修改”仅指实付金额与发票总金额不一致，其他字段的修改仍正常留痕，但不进入该视图。通过卡片勾选、Shift 范围选择和 Cmd/Ctrl+A 选中条目后，再做批量修改报账人、汇总、导出、打印或删除。
