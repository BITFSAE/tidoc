...
- 本地识别记录识别规则版本与材料摘要。任一列表视图选中条目后都可打开「重新识别」，分别勾选发票或付款截图；已经由当前规则处理且原文件未变化的材料自动跳过。付款截图重新识别只刷新识别结果与提醒，不覆盖用户已确认的实付金额。截图未识别到金额，或同一条目的截图识别金额合计与发票总额不一致时，合并为条目的非阻断「识别提醒」。
