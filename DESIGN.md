...
- 绑定包 `.tidoc`：给 tidoc 之间互相导入继续整理，保留报账人姓名 / 审核人、每条发票的归属、条目、附件、修改历史与签名校验。接收方按姓名 + 审核人复用已有报账人，否则创建新报账人；导入后报账人达到两个时自动开启代填模式。条目 / 附件备注和条目标签是否带出由设置中的两个独立开关控制，默认包含；关闭备注时也移除备注修改历史和汇总中的备注，避免从旁路带出。导入按整个包原子处理，任一条目、身份或附件失败时回滚本次全部写入；发票号或发票文件摘要已存在时跳过重复条目。用户明确确认接收完整性异常的包后，导入条目标记为严重问题，不以普通条目静默落库。
