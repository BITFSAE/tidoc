...
3. **核对与整理**：在任一列表视图选中条目后都可点「重新识别」，并分别选择发票或付款截图；已经由当前识别规则处理且原文件未变化的材料自动跳过。付款截图未识别到金额，或各截图识别合计与发票总额不一致时进入识别提醒；阿里云云识别用于补齐本地遗漏并提供第二份结果供比对（选中条目点「云识别」，或识别提醒视图一键识别全部）。多页发票会逐页识别并合并，确认框按实际页数显示调用次数。软件已有的销售方和明细值优先保留，规格型号差异忽略；明细的名称、单位、数量、金额或行数有任何差异时进入 OCR 待确认（云端为空的单元格与数量的小数位写法不算差异，云端整体未返回明细时只提示已保留软件明细、不进待确认）。多选云识别默认跳过当前发票已有完整结果的条目，也可取消跳过后重新识别。详情会列出实际自动补齐的前后值和未采用的云端差异。条目有多张付款截图时，卡片付款按钮用不同颜色显示张数，高级筛选可只看「多张（2 张及以上）」；也可按抬头、报账人、状态、日期、金额、标签和备注筛选。卡片上的报账人、批次可直接编辑，材料按钮右键可打开已有文件。批次栏用「在办 / 已归档」切换工作面，点击批次右侧「⋯」可编辑批次、填写批次备注或归档，已归档批次可恢复到在办。设置 → 扩展的「抬头与税号」可按学校或单位维护报账抬头和购买方税号（默认内置北理工与教育基金会），识别与校验按配置执行；同处可修改新建默认抬头，并分别设置付款截图、实物图、查验单和实付金额是否必需；发票固定必需，默认要求付款截图、查验单和实付金额，实物图可选。
4. **交换、导出与打印**：选中条目后可创建批次、导出 `.tidoc` 绑定包、总览 Excel 或附件整理包。绑定包导入前可预览并调整报账人、审核人、批次和标签，重复发票跳过并校验 HMAC；Windows 安装器会把 `.tidoc` 文件关联到 Tidoc，双击即可进入导入预览；阿里云识别结果和“已识别 / 待确认”状态会随包恢复。打印组件默认按条目合并发票、付款截图和查验单，也支持按材料分开导出，并生成报账说明、验收单 Word；若当前聚焦某个批次，会自动带入该批次已保存的批次备注；Word 与 PDF 按抬头隔离。

## 识别与数据规则

- 发票识别以 XML 优先、PDF 文本兜底，兼容常见数电发票的错位、分列明细和折扣行；缺明细、金额不闭合、抬头异常，以及已配置抬头（默认北理工 / 教育基金会）购买方税号缺失或不符等会形成识别提醒。
- 识别提醒与材料齐备是两套状态；严重问题会单独标出。全库按发票号或文件内容拦截重复发票，附件类型、发票归属和导入事务也会校验。
- 默认不依赖云端通用 OCR：系统 OCR 只用于付款截图金额和查验单发票号的轻量兜底，发票本身仍以 XML / PDF 文本解析为主。阿里云发票识别作为可选付费组件（见下），多页 PDF 拆页调用后按页序合并，并把紧随商品的空名称负数折扣行计入该商品；识别结果永久保存在条目里，可随时比对、采用，不会为查看结果重复计费。
...
- [x] Windows 安装器关联 `.tidoc` 文件（显示应用图标，双击进入导入预览）
