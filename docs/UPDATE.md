...
tidoc/ocr/windows/tidoc-ocr-windows-v0.1.0.exe
tidoc/ocr/macos/tidoc-ocr-macos-v0.1.0.zip
```
...
- OCR 识别组件只负责执行阿里云识别调用（`ocr-api.cn-hangzhou.aliyuncs.com`），密钥由用户在设置内自填并仅存本机；组件自检（`--self-test`）不联网。识别结果（含原始 JSON 和自动补齐 / 历史修正快照）落库在 `ocr_results` 表，与更新通道无关。Windows 核心以不创建控制台窗口的方式启动组件，避免批量识别逐张闪出终端黑框。
